<script>
	...
	import backIcon from '$lib/icons/back.png';
	import forwardIcon from '$lib/icons/forward.png';
	import playIcon from '$lib/icons/pause.png';
	import pauseIcon from '$lib/icons/play.png';
	...
	function play() {
		if (player && player.nextVideo) {
			if (player.getPlayerState() == 1) {
				player.pauseVideo();
				document.getElementById('pause').setAttribute('src', pauseIcon);
				updateData();
			} else if (player.getPlayerState() == 2 || player.getPlayerState() == -1) {
				player.playVideo();
				document.getElementById('pause').setAttribute('src', playIcon);
				updateData();
			}
		} else {
			console.error('player brokey');
...
<div class="fixed inset-0 w-screen h-screen z-[-1] bg items-center flex flex-col font">
	...
	<div class="m-4">
		<button on:click={previous} class="font bg-slate-900 p-7 rounded-3xl">
			<img src={backIcon} /></button
		>
		<button on:click={play} class="font bg-slate-900 p-7 rounded-3xl">
			<img id="pause" src={pauseIcon} /></button
		>
		<button on:click={next} class="font bg-slate-900 p-7 rounded-3xl">
	...
	<input
		...
		min="0"
		max="100"
		on:change={() => changeVolume(document.getElementById('volume').value)}
	/>
	<p class="text-center m-5">
		<label id="elapsed">0:00</label>
		<input
			class="w-96"
			type="range"
			id="seek"
			min="0"
			max="1"
			on:change={() => seek(document.getElementById('seek').value)}
			on:input={() => moveSeeker(document.getElementById('seek').value)}
